...
function mapped_quantum_chemistry_hamiltonian(E0, K, V, Elt = ComplexF64)
    ...
    for i in 1:basis_size, j in (i + 1):basis_size
        # p p . m | m
        hamdat[i, 1, indmap_1R[2, j]] += V[i, i, j, i] * pp_m

        # p p m . | m
        hamdat[i, 1, indmap_1R[2, j]] += V[i, i, i, j] * ppm_

        # . p m m | p
        hamdat[i, 1, indmap_1R[1, j]] += V[j, i, i, i] * _pmm

        # p . m m | p
        hamdat[i, 1, indmap_1R[1, j]] += V[i, j, i, i] * p_mm
    end
    ...
    for i in 1:basis_size, j in (i + 1):basis_size
        # m | p p . m
        hamdat[j, indmap_1L[2, i], end] += V[j, j, i, j] * pp_m

        # m | p p m .
        hamdat[j, indmap_1L[2, i], end] += V[j, j, j, i] * ppm_

        # p | . p m m
        hamdat[j, indmap_1L[1, i], end] += V[i, j, j, j] * _pmm

        # p | p . m m
        hamdat[j, indmap_1L[1, i], end] += V[j, i, j, j] * p_mm
    end
    ...
    for i in 1:basis_size, j in (i + 1):basis_size
        ...
        hamdat[i, 1, end] -= V[i, j, i, j] * MPSKit.add_util_leg(h_pm)
        ...
        # p m | p . . m
        p__m = _pm_
        hamdat[j, indmap_2L[2, i, 1, i], end] += V[j, i, i, j] * p__m
    end
    ...
    for i in 1:basis_size, j in (i + 1):basis_size, k in (j + 1):basis_size
        ...
        # p p L R
        hamdat[j, indmap_1L[2, i], indmap_1R[2, k]] += V[j, j, i, k] * LRmm
    ...
    # 2|1|1
    # (i,j) in indmap_2L, 1 in indmap_1R, 1 onsite
    for i in 1:basis_size, j in (i + 1):basis_size, k in (j + 1):basis_size
        # L L m R
        ...
        hamdat[j, indmap_2R[2, i, 1, i], indmap_1R[2, k]] += V[i, j, k, i] * LpRL

        # L R m L
        @plansor LRpL[-1 -2; -3 -4] := ut[-1] * am[-3 -2; -4]
        hamdat[j, indmap_2R[2, i, 1, i], indmap_1R[1, k]] += V[i, k, j, i] * LRpL

        # p L L R
        @plansor pLLR[-1 -2; -3 -4] := ut[-1] * ap[-3 -2; -4]
        hamdat[j, indmap_2R[2, i, 1, i], indmap_1R[2, k]] += V[j, i, i, k] * pLLR

        # R L L m
        @plansor RLLm[-1 -2; -3 -4] := ut[-1] * am[-3 -2; -4]
        hamdat[j, indmap_2R[2, i, 1, i], indmap_1R[1, k]] += V[k, i, i, j] * RLLm
    end

    # 1|1|2
    # (i,j) in indmap_2L, 2 onsite
    for i in 1:basis_size, j in (i + 1):basis_size, k in (j + 1):basis_size
        # j i m m
        ...
        hamdat[k, indmap_2R[2, i, 1, j], end] += V[j, k, k, i] * jpmi
        # i p m j
        @plansor ipmj[-1 -2; -3 -4] := ut[-1] * h_pm[-2; -3] * conj(ut[-4])
        hamdat[k, indmap_2R[1, i, 2, j], end] += V[i, k, k, j] * ipmj
        ...
        # p . . m
        pjim = jpmi
        hamdat[k, indmap_2R[2, i, 1, j], end] += V[k, j, i, k] * pjim
        pijm = ipmj
        hamdat[k, indmap_2R[1, i, 2, j], end] += V[k, i, j, k] * pijm
    ...
    # 1|1|1|1
    # (i,j) in indmap_2L, 1 in indmap_2R, 1 onsite

    for i in 1:basis_size, j in (i + 1):basis_size,
            k in (j + 1):basis_size, l in (k + 1):basis_size
        # p j i R
        @plansor pjiR[-1 -2; -3 -4] := ut[-1] * ap[-3 -2; -4]
        hamdat[k, indmap_2R[2, i, 1, j], indmap_1R[2, l]] += V[k, j, i, l] * pjiR

        # p i j R
        @plansor pijR[-1 -2; -3 -4] := ut[-1] * ap[-3 -2; -4]
        hamdat[k, indmap_2R[1, i, 2, j], indmap_1R[2, l]] += V[k, i, j, l] * pijR

        # R j i m
        @plansor Rjim[-1 -2; -3 -4] := ut[-1] * am[-3 -2; -4]
        hamdat[k, indmap_2R[2, i, 1, j], indmap_1R[1, l]] += V[l, j, i, k] * Rjim

        # R i j m
        @plansor Rijm[-1 -2; -3 -4] := ut[-1] * am[-3 -2; -4]
        hamdat[k, indmap_2R[1, i, 2, j], indmap_1R[1, l]] += V[l, i, j, k] * Rijm

        # j p R i
        @plansor jpRi[-1 -2; -3 -4] := ut[-1] * ap[-3 -2; -4]
        hamdat[k, indmap_2R[2, i, 1, j], indmap_1R[2, l]] += V[j, k, l, i] * jpRi

        # i p R j
        @plansor ipRj[-1 -2; -3 -4] := ut[-1] * ap[-3 -2; -4]
        hamdat[k, indmap_2R[1, i, 2, j], indmap_1R[2, l]] += V[i, k, l, j] * ipRj

        # j R m i
        @plansor jRmi[-1 -2; -3 -4] := ut[-1] * am[-3 -2; -4]
        hamdat[k, indmap_2R[2, i, 1, j], indmap_1R[1, l]] += V[j, l, k, i] * jRmi
    ...
    return th + fill(E0 / basis_size, basis_size), (indmap_1L, indmap_1R, indmap_2L, indmap_2R)
end
